  ...
  const UI = {
    ...
    init() {
      ...
      document.addEventListener('keydown', ev => {
        ...
        if (ev.key === 'Escape') { this.closeOverlay(); return; }
    ...
    refreshTitle() {
      ...
      x.classList.toggle('hidden', !E.extraUnlocked());
    ...
    setBg(id) {
      ...
      next.className = 'bg-slot ' + (this.bgSlot === 1 ? 'bg-b' : 'bg-a') + ' bg-fade-in';
      cur.classList.remove('bg-fade-in');
    ...
    openSettings() {
      ...
      wrap.innerHTML = `
        ...
        <label class="st-row"><span>雨声</span>
          <input type="checkbox" data-k="rain">
        </label>
        <label class="st-row"><span>音量</span>
          <input type="range" min="0" max="100" data-k="vol">
        </label>`;
      // 当前值
      wrap.querySelector('[data-k="speed"]').value = String(cfg.speed);
      wrap.querySelector('[data-k="auto"]').value = String(cfg.auto);
      wrap.querySelector('[data-k="rain"]').checked = !!cfg.rain;
      wrap.querySelector('[data-k="vol"]').value = Math.round(cfg.vol * 100);
      // 事件
    ...
    openGallery() {
      ...
      const endList = [
        ...
        { k: 'TRUEA', n: 'TRUE · A', cat: 'T' }, { k: 'TRUEB', n: 'TRUE · B', cat: 'T' },
        { k: 'TRUEC', n: 'TRUE · C', cat: 'T' }, { k: 'TRUED', n: 'TRUE · D', cat: 'T' }, { k: 'TRUEE', n: 'TRUE · E', cat: 'T' },
      ];
      endList.forEach(e => {
